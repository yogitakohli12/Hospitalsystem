...
function UpdateAccountant() {
  ...
  const updateAccountant = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`http://localhost:8000/updateAccountact/${email}`,{updateemail,updatename,updatephone,updateage,updatesalary,updatedepartment} );
      console.log(res);
      alert('Accountant is successfully updated 😊');
    } catch (error) {
  ...
  return (
    ...
    <Dashboard/>
        ...
        <form onSubmit={updateAccountant} id='updateform'>
        <div id='head'>
       <Link to="/accountants"><button  >Back</button></Link>
